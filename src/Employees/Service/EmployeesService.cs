...
public class EmployeesService
{
    ...
    public async Task<AllergiesAndDietaryPreferences?> GetAllergiesAndDietaryPreferencesByEmployee(
        EmployeeEntity employee)
    {
        var employeeAllergiesAndDietaryPreferences =
            await _employeeAllergiesAndDietaryPreferencesRepository.GetByEmployee(employee);

        return employeeAllergiesAndDietaryPreferences == null
            ? null
            : ModelConverters.ToAllergiesAndDietaryPreferences(employeeAllergiesAndDietaryPreferences);
    }
    ...
    public async Task<Cv> GetCvForEmployee(string alias, string country)
    {
        ...
        List<PresentationEntity> presentationEntities =
            await _employeesRepository.GetPresentationsByEmployeeId(employeeId);
        ...
        List<ProjectExperience> projectExperiences = new();
        foreach (ProjectExperienceEntity entity in projectExperienceEntities)
        {
            ...
            projectExperience.roles = await GetProjectRolesByProjectId(entity.Id);
    ...
    private async Task<List<ProjectExperienceRole>> GetProjectRolesByProjectId(string id)
    {
        var projectExperienceRole = await _employeesRepository.GetProjectRoleByProjectId(id);
        return projectExperienceRole.Select(entity => new ProjectExperienceRole()
        {
